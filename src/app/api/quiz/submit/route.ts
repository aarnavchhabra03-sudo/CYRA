...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 10.5 UPDATE LESSON COMPLETION IN user_progress AND RECALCULATE COURSE PROGRESS IF PASSED
    if (summary.passed && quizRecord.lesson_id) {
      try {
        console.log(`[QUIZ SUBMIT] Quiz passed! Marking lesson ${quizRecord.lesson_id} as completed.`);
        await adminClient
          .from('user_progress')
          .upsert(
            {
        ...
        if (learningPathId) {
          // Fetch all lessons belonging to this learning path
          ...
          if (allLessons && allLessons.length > 0) {
            ...
            const newPct = Math.min(100, Math.round((completedCount / allLessons.length) * 100));

            console.log(`[QUIZ SUBMIT] Recalculated course progress: ${newPct}% (${completedCount}/${allLessons.length} lessons completed)`);
            await adminClient
              .from('learning_paths')
        ...
        console.error('[QUIZ SUBMIT] Error updating lesson completion/progress:', progressErr);
      }
    }
