...
export async function GET(request: Request) {
  try {
    ...
    // Expose decay information for the Roadmap UI badge
    const decayedConcepts = snapshot.mastery
      .filter((m) => m.reviewNeeded && m.questionsAttempted > 0)
      .map((m) => ({
        concept: m.concept,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        state: {
          ...
          hasCriticalWeakness,
          hasBlockedConcept,
          hasDecay: decayedConcepts.length > 0,
        },
        decayedConcepts,
