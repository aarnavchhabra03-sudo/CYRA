    ...
    <div className="flex-1 p-8 max-w-4xl mx-auto w-full space-y-8">
                            ...
                            <span className="text-[11px] text-zinc-400 italic flex-1">
                              ...
                              >
                                {isGeneratingThis ? (
                                  ...
                                  <>
                                    <span>{isBlocked ? 'Fix Prerequisite First' : 'Practice Concept'}</span>
      ...
      <div className="os-card p-6 space-y-5 bg-[var(--cyra-panel)] border border-[var(--cyra-border)]">
        <div className="flex items-center justify-between border-b border-[var(--cyra-border)] pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-[var(--cyra-text)] tracking-wide font-mono uppercase">Concept Mastery Tracker</h3>
              <p className="text-xs text-[var(--cyra-text-secondary)] font-sans">Persistent analytics derived from your quiz attempts.</p>
            </div>
          </div>
        ...
        {loadingMastery ? (
          ...
          <div className="os-card p-6 text-center space-y-2 bg-[var(--cyra-card)] border border-[var(--cyra-border)]">
            ...
            <p className="text-xs text-[var(--cyra-text-secondary)]">No concept mastery records tracked yet.</p>
            <p className="text-[11px] text-[var(--cyra-text-muted)] font-mono">Complete an AI quiz to populate your personalized concept intelligence profile.</p>
          </div>
        ) : (
          <div className="space-y-6">
                ...
                <div key={courseTitle} className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {records.map((m) => {
                      ...
                      return (
                        <div key={m.id} className="os-card p-4 flex items-center justify-between gap-4 bg-[var(--cyra-card)] border border-[var(--cyra-border)]">
                          <div className="space-y-0.5">
                            <span className="text-xs font-bold text-[var(--cyra-text)] block font-sans">{m.concept}</span>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-24 bg-[var(--cyra-card-soft)] h-1.5 rounded-full overflow-hidden hidden sm:block">
                              <div 
                                className={`h-full ${
                                  isMastered ? 'bg-[var(--cyra-green)]' : isProficient ? 'bg-[var(--cyra-cyan)]' : isDeveloping ? 'bg-[var(--cyra-amber)]' : 'bg-[var(--cyra-red)]'
                                }`}
                                style={{ width: `${m.mastery_score}%` }}
                              />
                            </div>

                            <span className="text-xs font-mono font-bold text-[var(--cyra-text)]">{m.mastery_score}%</span>

                            <span className={`os-badge ${badgeClass}`}>
                              {m.last_result}
                            </span>
                          </div>
                        </div>
      ...
      <div className="os-card p-5 space-y-3 bg-[var(--cyra-panel)] border border-[var(--cyra-border)]">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-xs font-mono font-bold text-[var(--cyra-text)] uppercase">ADAPTIVE GAMIFIED ACHIEVEMENTS</h4>
            <p className="text-xs text-slate-400 mt-1 leading-relaxed font-sans">
              Maintain active study streaks to earn XP multipliers. Mastering concepts unlocks customized AI quiz formats and advanced practice modes.
            </p>
          </div>
        </div>
      </div>
