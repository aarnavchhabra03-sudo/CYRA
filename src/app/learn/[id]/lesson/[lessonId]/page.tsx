...
export default function LessonPage() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[var(--cyra-bg)] text-[var(--cyra-text)] flex items-center justify-center p-6 font-sans">
        <div className="p-8 rounded-2xl bg-[var(--cyra-panel)] border border-[var(--cyra-border)] text-center max-w-md space-y-4 shadow-md">
          <div className="w-12 h-12 rounded-full bg-rose-50 border border-rose-200 text-[#D45D6B] flex items-center justify-center mx-auto">
            <AlertTriangle className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-base font-bold text-[var(--cyra-text)] mb-1">Lesson Unavailable</h3>
            <p className="text-xs text-[var(--cyra-text-secondary)] leading-relaxed">{error.message}</p>
            <span className="inline-block mt-2 text-[9px] font-mono text-[var(--cyra-text-muted)] bg-[var(--cyra-card-soft)] px-2 py-0.5 rounded border border-[var(--cyra-border)]">
              Code: {error.code}
            </span>
          </div>
          <Link
            href={`/learn/${learningPathId}`}
            className="os-button-secondary inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold"
          >
