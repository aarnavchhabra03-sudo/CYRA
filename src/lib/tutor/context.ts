...
export interface TutorContext {
  ...
  masteryRetentionContext?: {
    decayedConcepts: Array<{
      concept: string;
      masteryScore: number;
      effectiveMasteryScore: number;
      daysSinceReview: number;
...
export async function buildTutorContext({
  ...
  try {
    ...
    if (masteryRows && masteryRows.length > 0) {
      ...
      // Group concepts by mastery level, enriching with decay metadata
      const decayedConcepts: Array<{
        concept: string;
        masteryScore: number;
        effectiveMasteryScore: number;
        daysSinceReview: number;
      ...
      for (const r of masteryRows) {
        ...
        // Collect decayed concepts for retention context (only assessed)
        if (r.questions_attempted > 0 && decay.effectiveMasteryScore < 85) {
          decayedConcepts.push({
            concept: r.concept,
            masteryScore: r.mastery_score,
            effectiveMasteryScore: decay.effectiveMasteryScore,
            daysSinceReview: Math.round(decay.daysSinceReview),
            urgency: decay.effectiveMasteryScore < 50 ? 'priority' : 'review',
          });
        }
      }

      if (decayedConcepts.length > 0) {
        context.masteryRetentionContext = { decayedConcepts };
      }
