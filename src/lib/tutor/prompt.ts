...
export function isAnswerExtractionAttempt(userMessage: string): boolean {
  ...
  const patterns = [
    /\b(what|tell|give|show|reveal|provide|get)\b.*\b(answers?|solutions?|correct\s+options?|right\s+options?|answer\s+keys?)\b/i,
    /\b(which\s+options?|which\s+one|which\s+choice|correct\s+choice|right\s+choice)\b/i,
    ...
    /\b(tell\s+me|show\s+me)\s+which\s+(one|option|letter|answer|answers)\b/i,
    /\b(what\s+is\s+the|what's\s+the)\s+answers?\b/i,
    /\b(give\s+me|tell\s+me)\s+the\s+answers?\b/i,
...
`;
  ...
  // Format mastery retention context
  const retCtx = context.masteryRetentionContext;
  let retentionSection = '';
  if (retCtx && retCtx.decayedConcepts.length > 0) {
    const priorityConcepts = retCtx.decayedConcepts.filter((c) => c.urgency === 'priority');
    const reviewConcepts = retCtx.decayedConcepts.filter((c) => c.urgency === 'review');

    const priorityList = priorityConcepts
      .map((c) => `- ${c.concept} (original: ${c.masteryScore}%, effective: ${c.effectiveMasteryScore}%, ~${c.daysSinceReview} days since review)`)
      .join('\n') || 'None';
    const reviewList = reviewConcepts
      .map((c) => `- ${c.concept} (original: ${c.masteryScore}%, effective: ${c.effectiveMasteryScore}%, ~${c.daysSinceReview} days since review)`)
      .join('\n') || 'None';

    retentionSection = `
============================================================
<MASTERY_RETENTION_CONTEXT>
The following concepts may have faded since the learner last studied them.

PRIORITY REVIEW (effective score < 50%):
${priorityList}

MODERATE REVIEW (effective score 50–84%):
${reviewList}

PEDAGOGICAL GUIDANCE:
- If the student asks about any of these concepts, gently note that they are due for a refresher.
- Offer to walk through core ideas again before building on them.
- Do NOT explicitly say "your memory is fading" or make the student feel anxious.
- Keep this advisory; do NOT force a review if the student is asking about something else.
</MASTERY_RETENTION_CONTEXT>
`;
  }
