...
/**
 * Deterministically calculates effective concept mastery with memory decay.
 */
export function calculateEffectiveMastery(
  ...
  if (questionsAttempted === 0) {
    return {
      effectiveMasteryScore: masteryScore,
      retentionFactor: 1.00,
      daysSinceReview: 0,
      reviewNeeded: false,
    };
  }

  const refDate = lastReviewedAt ? new Date(lastReviewedAt) : new Date();
  const daysSinceReview = Math.max(0, (Date.now() - refDate.getTime()) / (1000 * 60 * 60 * 24));

  let retentionFactor = 1.00;
  ...
  const effectiveMasteryScore = Math.max(0, Math.min(100, Math.round(masteryScore * retentionFactor)));
  const reviewNeeded = effectiveMasteryScore < 85;
 ...
 * Accepts optional pre-fetched mastery rows to prevent duplicate DB queries.
 */
export async function buildLearnerKnowledgeGraph(
  userId: string,
  learningPathId?: string | null,
  preFetchedMastery?: Array<{ concept: string; mastery_score: number; questions_attempted?: number; last_reviewed_at?: string | null }>
): Promise<KnowledgeGraphData> {
  try {
    const relationships = await getUserConceptRelationships(userId, learningPathId);

    let masteryRows = preFetchedMastery;
    if (!masteryRows) {
      let query = adminClient
    ...
    if (masteryRows) {
      for (const row of masteryRows) {
        ...
        // Use effectiveMasteryScore in the prerequisite graph so decay propagates through pre-req analysis
        const { effectiveMasteryScore } = calculateEffectiveMastery(
          row.mastery_score,
          (row as any).last_reviewed_at,
          row.questions_attempted ?? 0
        );
        masteryMap.set(norm, effectiveMasteryScore);
...
/**
 * Atomic PostgreSQL transaction helper for approving Research Knowledge Maps.
 * Merges concept relationships into public.concept_relationships AND sets
 * map status to 'approved' in a single atomic transaction.
 */
export async function approveAndMergeKnowledgeMap({
  userId,
  mapId,
  relationships,
  updatedNodes,
  ...
  if (!userId || !mapId) {
    return { success: false, error: 'User ID and Map ID are required.', code: 'INVALID_REQUEST' };
  }

  const formattedRelationships = (relationships || []).map((r) => ({
    source_concept: r.sourceConcept,
  ...
  try {
    const { data, error } = await adminClient.rpc('approve_research_knowledge_map', {
      p_map_id: mapId,
      p_user_id: userId,
      p_relationships: formattedRelationships,
      p_updated_nodes: updatedNodes,
      p_updated_edges: updatedEdges,
    });

    if (error) {
      console.error('[KNOWLEDGE GRAPH] RPC approve_research_knowledge_map error:', error);
      if (error.message?.includes('MAP_NOT_PENDING_OR_NOT_FOUND')) {
        return { success: false, error: 'Knowledge map is not pending approval or not found.', code: 'MAP_NOT_PENDING' };
      }
      return { success: false, error: 'Failed to merge knowledge map into learner graph', code: 'GRAPH_MERGE_FAILED' };
    }

    return { success: true, data };
  } catch (err: any) {
    console.error('[KNOWLEDGE GRAPH] Exception in approveAndMergeKnowledgeMap:', err);
    return { success: false, error: 'Failed to merge knowledge map into learner graph', code: 'GRAPH_MERGE_FAILED' };
  }
}
